...
fn main() {
    ...
    let time_sum = Arc::new(AtomicU64::new(0));
    ...
    roots.par_iter_mut().for_each(|root| {
        ...
        let elapsed_ns = time_total.elapsed().as_nanos() as u64;
        time_sum.fetch_add(elapsed_ns, Ordering::Relaxed);
    });

    let total_duration = time_total.elapsed().as_secs_f64();
    let total_time_sum = time_sum.load(Ordering::Relaxed);
    let total_time_sum = total_time_sum as f64 / 1e9;
    println!("Tree explored in {} and {} nodes", total_duration, total_nodes.load(Ordering::Relaxed));
    let _ = log_to_file(length, total_time_sum, total_nodes.load(Ordering::Relaxed));
